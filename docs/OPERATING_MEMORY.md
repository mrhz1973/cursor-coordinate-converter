...
- **Range Rings B6.6C panel restore after create (2026-06-20):** PASS tecnico — runtime **`41f180b`**; build label **`B6.6C`**; hook `openRangeRingsFloatingPanelGis()` incondizionato dopo `rrCreateFromUi()` nel handler map-click pick-and-create (~L33658); restore utile anche su validazione fallita (distanze vuote); `rrCreateFromUi()` invariata; export JPG/scala/B6.6B handle non toccati; **`node --check`** OK; **QA operatore post-deploy VPS: pending**.
