...
- **WU-0007 B6.7a Range Rings titolo opzionale + Stili comprimibile (2026-06-22):** implementato locale — proprietà per-ring **`showTitle`** (fallback legacy da `labelMode === "name"` in `sanitizeRangeRingSet`); checkbox **`#rrShowTitle`** in sezione Stili; gate render **`rrSetShowsTitle`** (titolo mappa indipendente da etichette distanza); sezione **`#rrStyleDetails`** (`<details>` chiuso di default, pattern `.rr-io-details`) raggruppa stile anelli/label e spokes; **`labelMode`** conservato; import/export GeoJSON invariati; **`APP_BUILD_ID`** invariato (`B5.5Z`); **B6.7b** memoria ultimo stile persistente **backlog, non implementato**; **QA operatore pending**.
