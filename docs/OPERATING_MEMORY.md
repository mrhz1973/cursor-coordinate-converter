...
- **WU-0007 T1-FLOAT — float Traccia allineato a `trackDisplayUnit` — runtime implementato; QA operatore pending:** `updateTrackMapFloatReadout()` e picker `[data-role="track-float-unit"]` usano `normalizeTrackDisplayUnit(state.trackDisplayUnit)` + `formatTrackDistance` (non più `mapMeasureUnit` / `formatMapMeasureDistance`); opzioni float **km / NM / mi** (rimossi `m` e `ft`); cambio unità nel float scrive `trackDisplayUnit`, `saveStore()` e aggiorna solo il readout float — **nessun** side-effect su Misura (`mapMeasureUnit`, `renderMapMeasureOverlay`, `updateMeasureReadouts`); modal Traccia, archivio, ETA e listener Misura invariati; float mostra **solo distanza** (nessuna velocità); **`APP_BUILD_ID` `B5.5Z` invariato**; deploy VPS **non eseguito** in questo blocco; **QA operatore T1-FLOAT: non attestata**.
