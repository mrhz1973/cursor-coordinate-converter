...
- **Measurement label collision avoidance — WU-0007 B4X1/B4X2:** B4X1 implementato **`debd5b4`** — QA operatore **FAIL** (pill/background desincronizzato; offset eccessivo segmenti quasi nulli). **B4X2** implementato runtime (SHA da `finito`) — `layoutPill()` unifica rect+offset da `mmMeasurePillDimsFromTextEl`; tuning offset (`CLEAR_GAP` 3, handle 18, gain 0.20, cap 84). **B4 righello PASS** (`54d8586`) invariato; **`APP_BUILD_ID` `B5.5Z`** invariato — QA via `?v=<runtime-short-sha>`. **QA operatore B4X2: pending**. Dettaglio: roadmap §WU-0007 B4X1/B4X2.
