...
- **Range Rings B6.4a-2 panel full-height (2026-06-20):** PASS tecnico statico — `_rangeRingsPanelLayoutOpts`: `defaultHeightFraction` **0.92**, `defaultHeightCap` **100000**, `topbarReserve` **104** → altezza da `fromReserve`; bottom-anchor `gisPanelDefaultRect` posiziona top sotto topbar (fix cap basso 640/720); build label **`B6.4a-2`** (`APP_BUILD_ID`, footer, About, title); **`node --check`** OK. **Browser QA operatore:** pending post-deploy VPS `:8000` (QA locale mappe tailscale non affidabile).
- **Prossimo candidato Range Rings:** backlog post-B6.4a (es. B6.5 drag centro, restore pannello post-create se richiesto) — da scegliere dopo QA operatore B6.4a-2.
