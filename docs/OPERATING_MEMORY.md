...
- **B5.5Z-BUILD label runtime visibile (2026-06-22):** PASS tecnico statico — **`APP_BUILD_ID`** aggiornato **`B5.5D` → `B5.5Z`**; **`APP_BUILD_DETAIL`** allineato a quick JPG geografico + segmentazione alto zoom; placeholder footer/about allineati; modifica **esclusivamente identificativa** (5 righe, nessuna logica export/tile/cache/OPSEC/provider/GPS/i18n); commento storico `(B5.5D)` su `drawJpgExportCoords` invariato; **`node --check`** OK; deploy VPS e smoke etichetta build **pending**. **Prossimo candidato:** deploy GIS-only + smoke label `B5.5Z` su tailnet.
