...
- **WU-0007 T1 — unità distanza/velocità modal Traccia — implementato runtime; QA operatore pending:** selettore persistente `state.trackDisplayUnit` (`km` | `nm` | `mi`, default `km`) sopra `#track-summary`; helper display `formatTrackDistance` / `formatTrackSpeed`; distanza totale, segmenti, leg chiusura e colonna archivio `#savedTracksMount` disaccoppiati da `mapMeasureUnit`; etichette ETA cammino/auto con velocità di riferimento dinamica (`km/h` | `kn` | `mph`) — calcolo temporale ETA invariato (4/60 km/h fisici); dati canonici in metri invariati; normalizzazione UI visibile `NM` (Misura + scala + helper misura); float esterno `updateTrackMapFloatReadout()` fuori scope T1; **`APP_BUILD_ID` `B5.5Z` invariato**; `node --check` OK.
