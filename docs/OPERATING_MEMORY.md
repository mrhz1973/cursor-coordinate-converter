...
- **WU-0009B B4 Bing Satellite (`bsat`) — B4.1A/B4.1A-VPS smoke PASS:** diagnostica quadkey + smoke statico strategia C dal VPS (2026-06-16), senza API key. **Contratto naming frontend GIS (fissato):** layer id `bsat`; route proxy prevista `/bsat/{z}/{x}/{y}.jpg`; provider consenso `bing`; flag `state._bingConsentGranted`; label UI «Bing Satellite». **Quadkey:** Bing canonico su `{z,x,y}` slippy; **non** sottrarre 1 allo zoom; trappola GetZ-1 SASPlanet isolata — non replicarla. **Smoke VPS:** `BING_STATIC_VERSION` known-good rilevato **15568** (2026-06-16); valore datato per riferimento deploy/drop-in systemd — **non** nel sorgente; soggetto a drift; riverifica futura **z19+**. Citazione `15568` in OM = audit trail documentale, non config viva (il divieto valori operativi hardcoded riguarda codice/sorgente e segreti). **Insidia:** parametro `g` può essere ignorato a zoom bassi/medi; validità di `g` va verificata a **z19+** nei refresh futuri, non a z10. **B4.2 frontend GIS:** layer `bsat` integrato nel monolite (mirror `gsat`: catalogo, gate OPSEC, consenso Bing separato, UI Layers, offline-eligible); **PASS tecnico statico**; **Browser QA OPSEC strict pending B4.4**. Proxy Planet-Clone deploy **`1e8944d`**. **Prossimo:** B4.4 Browser QA; spec proxy interna resta in Planet-Clone.
