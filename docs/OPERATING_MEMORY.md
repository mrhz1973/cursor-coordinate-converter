...
5. **QA operatore** sull'app deployata: tailnet `:8000` con `?v=<hash runtime>`; esito PASS/FAIL operatore;
6. **registrazione in OM §7:** hash runtime, HEAD deploy, smoke, link QA, esito PASS/FAIL operatore.
...
- **B5.4eB scala in-app allineata a export JPG (2026-06-20):** PASS tecnico statico — runtime **`0edf503`**; build **`B5.4eB`**; `buildScaleBar` layout due colonne (ratio **`1:N`** sx centrata, tabella metrica/Nm dx); CSS `.tile-scale` box opaco coerente export; mid-label rimosse; degradazione Nm su narrow (CSS); **`drawJpgExportScale`/export JPG invariati**; **`node --check`** OK; **QA operatore pending** post-deploy VPS (parità in-app vs JPG + regressione export B5.4d + Range Rings B6.6B).
