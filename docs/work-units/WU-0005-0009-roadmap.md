...
**B5.5Z-1 quick export — snapshot viewport e zoom dinamico (2026-06-21):** PASS tecnico statico — `getQuickExportViewportSnapshot()` + `computeQuickExportZoomLevels()`; infrastruttura read-only; antimeridiano fail-closed (pixel mondiale + `w > e`); limiti esistenti offline-export; nessun call site; nessun cambiamento UI/runtime; **`node --check`** OK; deploy/QA **non eseguiti**. **B5.5Z non completato.** **Prossimo candidato:** **B5.5Z-2** — estrazione motore geografico condiviso, Mappe Offline invariato.
