...
- **B5.5B-1 export JPG — overlay style fidelity (2026-06-21):** PASS tecnico statico — build **`B5.5B-1`**; fix QA FAIL parziale B5.5B: `rasterizeSvgOntoCanvas(..., inlineStyles=false)` — opt-in **`inlineStyles=true`** solo loop overlay B5.5B; `getComputedStyle` su elementi **live** → inline su clone (tag mirati: path/polyline/polygon/line/circle/rect/ellipse/text/tspan); griglia `.tile-grid` e marker `.tile-marker svg` invariati (6 arg, default false). Causa: serializzazione SVG senza CSS → fill nero/label senza paint-order. **`node --check`** OK. **QA operatore post-deploy VPS: pending** (re-deploy + `?v=<hash>`).
