...
- **WU-0008 8d-B EOX:** **PASS runtime** (pending review) — layer `eoxS2Cloudless` online-only (`cacheable:false`, `maxZoom:18`); WMTS REST `GoogleMapsCompatible` via `urlBase`+`tileScheme:zyx`+`urlSuffix:.jpg`; gate fetch fail-closed allowlist (`isPrivateEoxHostAllowed` / `eoxLayerAllowedOnCurrentHost` in `tileFetchAllowed`); UI Layers Satellitare solo host privato; CC BY-NC-SA attribution. Dettaglio: [`docs/work-units/WU-0005-0009-roadmap.md`](work-units/WU-0005-0009-roadmap.md) §8d-B.
- **Backlog metodo (post-catena 8d):** candidato «Adozione metodo / handoff discipline» — dettaglio in roadmap §Backlog metodo; non WU tecnica aperta.
