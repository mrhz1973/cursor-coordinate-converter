...
- **WU-0007 B6.7b Range Rings memoria ultimo stile persistente (2026-06-22):** **implementato runtime** — preferenza additiva `settings.rangeRingsLastStyle` in `coordconv_v2`; helper `sanitizeRangeRingsLastStyle`; nuovo ring via `rrGetPreferredNewRingFormStyle()` / `rrResetRangeRingsFormToNew()`; aggiornamento solo su create/save riusciti (`rrPersistRangeRingsLastStyleFromSet`); cancel/import/reset bozza non alterano la preferenza; ring esistenti invariati; `opacity` e `showTitle` inclusi (default nuovo ring `showTitle: true` senza preferenza); **`APP_BUILD_ID` `B5.5Z` invariato**; **QA operatore pending**.
