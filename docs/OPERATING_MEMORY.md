...
- **Offline UX / cache per-layer / maxZoom (WU-0008 8d-B1):** **8d-B1-A PASS** (diagnosi) + **8d-B1-A-docs PASS** (roadmap debiti). **8d-B1-B1 PASS** runtime (monolite, sessione `finito` 2026-06-15) — `isLayerOfflineUnavailable(layerId)` predicato unico catalogo-driven; badge «no offline» in menu Layers (i18n IT/EN/FR); `#pcLayer` da `OFFLINE_LAYER_IDS` derivato (`cacheable !== false`); pannello offline neutro; contatore sessione cache nascosto su basemap online-only; layer attivo menu in verde chiaro; debito `OFFLINE_LAYER_IDS` **risolto**; `node --check` OK; QA operatore badge/contatore PASS. **Prossimo candidato:** **8d-B1-B2** stats cache per-layer oppure **8d-B** EOX (prerequisito OFFLINE_LAYER_IDS soddisfatto). Resta debito fit-area z18 (B1-B3 zoom-guard).
