...
- **B5.5B export JPG avanzato — overlay mappa base (2026-06-20):** PASS tecnico statico — build **`B5.5B`**; dialog `#jpgExportDialog`: checkbox **`#jpgExportIncludeOverlays`** («Includi overlay mappa», default **true**, transiente `_jpgExportIncludeOverlays`, no localStorage); `exportMapAsJpg({ includeScale, includeOverlays })`; cattura WYSIWYG overlay SVG live via `rasterizeSvgOntoCanvas` (ordine: range rings → tracce → poligoni → waypoint) tra `.tile-grid` e `.tile-marker svg`; solo DOM live, nessun render forzato; `.tile-readout`/cursore **esclusi**; `drawJpgExportScale`/B5.4d/B5.4eB **invariati**; no fetch/cache/proxy; no zoom reale/2×/3×/tab coordinate (B5.5C–Z). **`node --check`** OK (2× script inline); no `<script src>`/`type="module"`. **QA operatore post-deploy VPS: pending.** Deploy VPS GIS-only + `?v=<hash runtime>` + checklist overlay ON/OFF + scala + regressione B6.6C.
