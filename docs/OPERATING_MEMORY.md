...
- **B5.5Z-FIX0 offline-export JPG — `layer` undefined (2026-06-21):** PASS tecnico statico — bug preesistente in `exportOfflineAreaAsJpg()`: `layer` letto senza dichiarazione locale in strict mode (`ReferenceError` atteso al primo accesso ~20519); fix: `const layer = getOfflineTileLayer(layerId)` subito dopo `getCurrentOfflineLayerId()`; accessor canonico; **nessuna** modifica a cache, rete, OPSEC, progress, placeholder o download; **`node --check`** OK; **deploy/QA browser non eseguiti**; prerequisito tecnico di **B5.5Z** (overlay offline-export), **non** apertura implementazione overlay; **B5.5Z non completato**.
