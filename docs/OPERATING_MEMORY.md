...
- **B5.5E-1 export JPG — default qualità massima 3× (2026-06-21):** PASS tecnico statico — build **`B5.5E-1`**; `_jpgExportScale` default **3** (sessione transiente, no localStorage); selettore 1×/2×/3× invariato; radio 3× preselezionato all'apertura dialog; scelta manuale 1×/2× persistente in sessione; supersampling/cap 8192/downgrade fail-safe B5.5E invariati; overlay `inlineStyles` B5.5B-1 invariato. **`node --check`** OK. **QA operatore post-deploy VPS: pending.**
