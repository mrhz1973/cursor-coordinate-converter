...
6. **WU-0010 — Outdoor Routing GraphHopper** (programma corrente, **OPEN / B1a–E + REVERSE-A + ELEVATION-STYLE-A + TRACK-ELEVATION-PROFILE-A + TRACK-SAVE-AS-NAME-A + ROUTING-SUMMARY-DEDUP-A + ROUTING-UX-POLISH-BUNDLE-A (+ FIX1) + APP-BUILD-LABEL-UX-A (+ FIX1) + TRACK-PROFILE-POINTS-DISPLAY-A + MAP-CENTER-VIEWPORT-AWARE-A (+ FIX1–FIX3) + ROUTING-POINT-COORD-EDIT-A (+ FIX1) + ROUTING-GEOCODING-MULTIROW-A (+ FIX1 + FIX2) + ROUTING-ALTERNATIVE-ROUTES-A (+ FIX1–FIX3) + ROUTING-GEOCODE-SNAP-A + UX-SEARCH-ERROR-FOCUS-A + ROUTING-MODAL-OPEN-EXPANDED-A (+ FIX1) + UI-MODAL-ERROR-FOCUS-A (+ FIX1 + FIX2) + ROUTING-ANDATA-RITORNO-A + ROUTING-ACTION-ROW-UX-A + ROUTING-ANELLO-A (+ FIX1) + OUTDOOR-ROUTING-F-AVOID-AREAS-A (+ FIX1) + OUTDOOR-ROUTING-ORS-PROVIDER-A + OUTDOOR-ROUTING-F-PROVIDER-COMPARE-A (+ FIX1–FIX6) CLOSED**; resto Bundle F **NOT OPENED**) — vedi [`WU-0010-outdoor-routing-graphhopper.md`](WU-0010-outdoor-routing-graphhopper.md). **INFRA-GH-1A/1B/1D CLOSED / PASS**; GraphHopper VPS live **V3** (`nord-ovest-B-v3-elev`). Runtime live monolite **`c5bc4b1`** / **`OUTDOOR-ROUTING-F-PROVIDER-COMPARE-A-FIX6 · build 228`**. **QA-OPERATOR-IT-ONLY-PREF CLOSED** (docs-only). **Oggetti GIS FROZEN**. Backlog non aperto: resto **Bundle F**, ….
...
- Blocchi futuri:
  ...
  - **TRACK-STYLE-A (+ FIX1 + FIX2) build 38 — CLOSED / PASS end-to-end:** runtime tip **`40c97b6`** (catena `ab5455d`→`1146e59`→`40c97b6`); blob **`2acf7711…`**; byte **2655368**; SHA-256 **`952550ef…`**; stile saved tracks (color/width/opacity/lineStyle) + editor singolo/batch + FIX lifecycle/targeting + FIX sync Include; bundle **DELICATO**; deploy PASS (operatore fuori Cursor); **QA PASS** «**QA TRACK-STYLE-A PASS operatore**»; **`APP_BUILD_NUM = 38`**.
  - **TRACK-BRUSH-A (+ FIX1 + FIX2 + FIX3) build 42 — CLOSED / PASS end-to-end:** catena **`15f9640`** (39) → **`75a1d5c`** (40) → **`db10408`** (41) → tip **`d4f877a`** (42); blob **`6e676089…`**; byte **2728773**; SHA-256 **`3660ce50…`**; pennello freehand → review → `savedTrackAddFromPoints` + read-back/rollback storage; lock import token-based; Esc non distruttivo in review; bundle **DELICATO**; review GPT-sostitutiva PASS; deploy GIS-only Cursor SSH PASS; **QA PASS** «**QA TRACK-BRUSH-A PASS operatore**»; **`APP_BUILD_NUM = 42`**. Superseded live da **TRACK-BRUSH-ANTIMERIDIAN** `9cc7937`.
  - **TRACK-BRUSH-ANTIMERIDIAN (+ FIX1) build 44 — CLOSED / PASS end-to-end:** catena **`bebf517`** (43) → tip **`9cc7937`** (44); blob **`6f22b7e9…`**; byte **2733148**; SHA-256 **`91272498…`**; shortest-wrap render + lift reject brush; FIX1 fit unwrap ordinato; bundle **DELICATO**; review downstream PASS; deploy GIS-only Cursor SSH PASS; **QA PASS** «**QA TRACK-BRUSH-ANTIMERIDIAN PASS operatore**»; **`APP_BUILD_NUM = 44`**. Superseded live da **TRACK-CREATE-EDIT-UX-A** `793f4cb`.
  - **TRACK-CREATE-EDIT-UX-A (+ FIX1) build 46 — CLOSED / PASS end-to-end:** catena **`33dc33d`** (45) → tip **`793f4cb`** (46); blob **`0afb9c91…`**; byte LF **2765139**; SHA-256 LF **`61c8b386…`**; CTA Nuova traccia punti/pennello; stile review brush + Modifica unificata; FIX1 verify persistenza canonica + rollback; bundle **DELICATO**; review downstream FIX1 PASS; deploy GIS-only Cursor SSH PASS; **QA PASS** «**QA TRACK-CREATE-EDIT-UX-A + FIX1 PASS operatore**»; **`APP_BUILD_NUM = 46`**. Superseded live da **OFFLINE-DOWNLOAD-CONTROLS-A** `fb11986`.
  - **OFFLINE-DOWNLOAD-CONTROLS-A (+ FIX1 + FIX2 + FIX3) build 50 — CLOSED / PASS end-to-end:** catena **`e130a6e`** (47) → **`5426cb1`** (48) → **`ede0215`** (49) → tip **`fb11986`** (50); blob **`2e31c335…`**; byte LF **2788844**; SHA-256 LF **`5d54aee7…`**; Pause/Resume/Stop; guard delete busy; sticky job bar; scroll unico Layers; thead statico + tabella `auto` min-width 980px; bundle **DELICATO**; review Claude PASS (A+FIX1–3); deploy GIS-only Cursor SSH PASS tip **`fb11986`**; **QA PASS** «**QA OFFLINE-DOWNLOAD-CONTROLS-A + FIX1 + FIX2 + FIX3 PASS operatore**»; **`APP_BUILD_NUM = 50`**. Superseded live da **MAJOR-3-b1** `1812010`.
  - **MAJOR-3-b1 build 51 — CLOSED / PASS end-to-end:** tip **`1812010`**; blob **`ba2cf240…`**; byte LF **2815080**; SHA-256 LF **`a3032d8f…`**; Workbench `#wbImportHub` preview GPX/KML/KMZ/GeoJSON zero-write; Mission Package rifiutato; lock hub; gate statico no-write PASS; review GPT PASS; deploy GIS-only Cursor SSH PASS; **QA PASS** «**QA MAJOR-3-b1 PASS operatore**»; **`APP_BUILD_NUM = 51`**. **MAJOR-3-b2 (+ FIX1)** CLOSED tip **`cad28e7`**.
  - **MAJOR-3-b2 (+ FIX1) build 98 — CLOSED / PASS end-to-end:** catena **`4d70bbc`** (97) → tip **`cad28e7`** (98 FIX1); documentale tip **`80265c3`** (AUTO-VIA, monolite invariato); blob **`ca931d93…`**; byte LF **3195195**; SHA-256 LF **`177c9cb1…`**; Apply anteprima Import Hub additivo all-or-nothing + FIX1 verify persistenza; Mission Package rifiutato; bundle **DELICATO**; harness 90/90; review+deploy GIS-only PASS; **QA PASS** «**QA MAJOR-3-b2 PASS operatore**»; display **`B6.4IHA-B2-FIX1 · build 98`**.
  - **OUTDOOR-ROUTING-ELEVATION-STYLE-A build 78 — CLOSED / PASS end-to-end:** tip **`d28bc44`**; blob **`e9ae353…`**; byte LF **3050747**; SHA-256 LF **`8e94e77a…`**; restyle visuale profilo altimetrico (segmenti per `b.gradePct`, griglia, A/B, tooltip); bundle ROUTINE; deploy GIS-only PASS; **QA PASS** «**QA OUTDOOR-ROUTING-ELEVATION-STYLE-A PASS operatore**»; display **`B6.0ES-A · build 78`**. Superseded live da **TRACK-ELEVATION-PROFILE-A** `1fc9d70`.
  - **TRACK-ELEVATION-PROFILE-A (+ FIX1 + FIX2 + FIX3) build 82 — CLOSED / PASS end-to-end:** catena **`4fb0d5a`** (79) → **`45bbf57`** (80 FIX1) → **`ae9ca1e`** (81 FIX2) → tip **`1fc9d70`** (82 FIX3); blob **`fd6f6ecc…`**; byte LF **3121652**; SHA-256 LF **`251dfad4…`**; profilo Saved Track + map-sync geometria canonica; **QA FAIL FIX2** storica → FIX3; review GPT-sostitutiva FIX3 PASS; deploy GIS-only PASS; **QA PASS** «**QA TRACK-ELEVATION-PROFILE-A-FIX3 PASS operatore**»; display **`B6.1TP-A-FIX3 · build 82`**.
  - **TRACK-MODAL-DISPLAY-PREFS-A build 77 — CLOSED / PASS end-to-end:** tip **`1e218a2`**; blob **`8ef3e171…`**; byte LF **3038595**; SHA-256 LF **`27f646a1…`**; unità distanza `m`/`ft` + formato coordinate session-only (Predefinito/DD/DDM/DMS/UTM/MGRS); solo display; review GPT-sostitutiva PASS; deploy GIS-only PASS; **QA PASS** «**QA TRACK-MODAL-DISPLAY-PREFS-A PASS operatore**»; display **`B6.0TDP-A · build 77`**. Superseded live da **ELEVATION-STYLE-A** poi **TRACK-ELEVATION** `1fc9d70`.
  - **OUTDOOR-ROUTING-REVERSE-A build 76 — CLOSED / PASS end-to-end:** tip **`d54c915`**; blob **`5c79d266…`**; byte LF **3033162**; SHA-256 LF **`9643ed48…`**; Inverti percorso (swap A/B + reverse via; ID stabili; invalidate preview; no auto-route); deploy GIS-only PASS; **QA PASS** «**QA OUTDOOR-ROUTING-REVERSE-A PASS operatore**»; display **`B6.0R-A · build 76`**. Superseded live da **TRACK-MODAL** poi **ELEVATION-STYLE-A**.
  - **OUTDOOR-ROUTING-GH-E (+ FIX1–FIX8) build 75 — CLOSED / PASS end-to-end:** catena **`e3cf114`** (67) → … → tip **`e7d9398`** (75 FIX8); blob **`df09e9dc…`**; byte LF **3029257**; SHA-256 LF **`1f7e2a7f…`**; profilo altimetrico SVG + difficoltà + sync chart↔mappa + locale numerico; **QA FAIL #1** altimetrico/pointer (post FIX4) → FIX5–FIX7; **QA FAIL #2** locale IT `3.8km` (post FIX7) → FIX8; review GPT-sostitutiva E+FIX1–FIX8 PASS; deploy GIS-only FIX8 PASS tip **`e7d9398`**; **QA PASS** «**QA OUTDOOR-ROUTING-GH-E PASS operatore**»; display **`B6.0E-FIX8 · build 75`**. Superseded live da **REVERSE-A** → **TRACK-MODAL** → **ELEVATION-STYLE-A**.
  - **OUTDOOR-ROUTING-GH-D (+ FIX1) build 66 — CLOSED / PASS end-to-end:** catena **`c806099`** (65) → tip **`567b611`** (66 FIX1); blob **`4f679f5b…`**; byte LF **2945471**; SHA-256 LF **`cd1c86e3…`**; Salva percorso come traccia (create-path + storage + rollback); FIX1 harden doppio clic / snapshot fail-closed / predicato busy; review GPT-sostitutiva D+FIX1 PASS; deploy GIS-only Cursor SSH PASS tip **`567b611`**; **QA PASS** «**QA OUTDOOR-ROUTING-GH-D PASS operatore**»; display **`B6.0D-FIX1 · build 66`**. Superseded live da catena successiva fino a **ELEVATION-STYLE-A** `d28bc44`.
  - **OUTDOOR-ROUTING-GH-C (+ FIX1) build 64 — CLOSED / PASS end-to-end:** catena **`61b5b34`** (63) → tip **`dd9ad2f`** (64 FIX1); blob **`a650c1c6…`**; byte LF **2940001**; SHA-256 LF **`9442a65f…`**; provider Local/VPS/Auto + `/info` + consenso loopback; FIX1 init A/B only; review GLM PASS + GPT-sostitutiva FIX1 PASS; deploy GIS-only Cursor SSH PASS tip **`dd9ad2f`**; **QA PASS** «**QA OUTDOOR-ROUTING-GH-C PASS operatore**»; display **`B6.0C-FIX1 · build 64`**. Superseded live da catena successiva fino a **ELEVATION-STYLE-A**.
  - **OUTDOOR-ROUTING-GH-B2 (+ FIX1 + FIX2) build 62 — CLOSED / PASS end-to-end:** catena **`42b01b3`** (60) → **`feb1eb3`** (61 FIX1) → tip **`89bbf28`** (62 FIX2); blob **`83da60d9…`**; byte LF **2916874**; SHA-256 LF **`041469ae…`**; GraphHopper POST `/route` + preview; review GPT-sostitutiva pre-deploy PASS; deploy GIS-only Cursor SSH PASS tip **`89bbf28`**; **QA PASS** «**QA OUTDOOR-ROUTING-GH-B2 PASS operatore**»; display **`B6.0B2-FIX2 · build 62`**. Superseded live da catena successiva fino a **ELEVATION-STYLE-A**.
  - **OUTDOOR-ROUTING-GH-B1b (+ FIX1) build 56 — CLOSED / PASS end-to-end:** catena **`3fc67c7`** (55) → tip **`3a702e1`** (56 FIX1); blob **`15c57074…`**; byte LF **2868398**; SHA-256 LF **`789eaefd…`**; pick/marker/GPS + disarmo Routing↔BBOX; review PASS; deploy GIS-only Cursor SSH PASS tip **`3a702e1`**; **QA PASS** «**QA OUTDOOR-ROUTING-GH-B1b PASS operatore**»; **`APP_BUILD_NUM = 56`**. Superseded live da **OUTDOOR-ROUTING-GH-B2** `89bbf28`.
  - **OUTDOOR-ROUTING-GH-B1a (+ FIX1 + FIX2) build 54 — CLOSED / PASS end-to-end:** catena **`53e25d6`** (52) → **`3760c77`** (53) → tip **`d95f745`** (54); blob **`06c83dff…`**; byte LF **2843944**; SHA-256 LF **`e7f985dc…`**; shell planner no-map; **QA PASS** «**QA OUTDOOR-ROUTING-GH-B1a PASS operatore**»; **`APP_BUILD_NUM = 54`**. Superseded live da **B1b** poi **B2** `89bbf28`.
  - **TRACK-POINT-CAP-2000 (+ FIX1 + FIX2) build 59 — CLOSED / PASS end-to-end:** catena **`249df83`** (57) → **`c94297f`** (58 FIX1) → tip **`ff43878`** (59 FIX2); blob **`db0d669…`**; byte LF **2887395**; SHA-256 LF **`eb92eedc…`**; cap 2000 fail-closed; verify+rollback create; paginazione lista 50; notice GIS import cap; review FIX2 PASS CON OSSERVAZIONI; deploy GIS-only Cursor SSH PASS (HEAD doc `f584b5f`, monolite=`ff43878`); **QA PASS** «**QA TRACK-POINT-CAP-2000-FIX2 PASS operatore**»; **`APP_BUILD_NUM = 59`**. Superseded live da **OUTDOOR-ROUTING-GH-B2** `89bbf28`.
  - **INFRA-GH-1D — CLOSED / PASS end-to-end (2026-07-29):** cutover VPS MMAP+V3 (`nord-ovest-B-v3-elev`; bilinear+ramer max_elevation 5; downtime 11 s; QA «**QA INFRA-GH-1D-EXEC-C PASS operatore**»); V0+backup trattenuti; monolite all’epoca `567b611`; **finito Regola H**. Bundle E sbloccato e **poi CLOSED** tip `e7d9398`.
  - **ROUTING-PROFILE-EDIT-A — SUPERSEDED / RENAMED — NO RUNTIME**. **QA-OPERATOR-IT-ONLY-PREF — CLOSED / PASS docs-only** (2026-08-01; Regola D1). **QA-CHATGPT-3LINE-HANDOFF-PREF — CLOSED / PASS docs-only** (2026-08-02; Regola D2 — QA via ChatGPT, Dove/Azione/Risultato atteso). **QA-CHATGPT-3LINE-CURSOR-RULES-A — CLOSED / PASS docs-only** (2026-08-02; `.cursor/rules` allineate a Regola D2). **Oggetti GIS — FROZEN**.
  - **QA-OPERATOR-IT-ONLY-PREF — CLOSED / PASS docs-only (2026-08-01):** QA IT + etichette UI visibili; freeze pannello **Oggetti GIS**; monolite invariato; deploy non richiesto.
  - **TRACK-POINT-CENTER-BUTTON-A build 96 — CLOSED / PASS end-to-end:** tip storico **`0482ef8`**; blob **`4f121880…`**; byte LF **3164587**; SHA-256 LF **`e77ad65e…`**; Centra per-riga Track Builder (ID stabile + `gisMapCenterOnLatLon`); ROUTINE; harness 31/31; deploy GIS-only PASS; **QA PASS** «**QA TRACK-POINT-CENTER-BUTTON-A PASS operatore**»; display **`B6.3TPC-A · build 96`**; superseded live da **MAJOR-3-b2** tip **`cad28e7`**.
  - **ROUTING-POINT-COORD-EDIT-A (+ FIX1) build 95 — CLOSED / PASS end-to-end:** tip storico **`6475804`**; catena `f509125`→`6475804`; blob **`a87920fe…`**; byte LF **3162728**; SHA-256 LF **`559795bf…`**; Modifica coordinate DD + FIX1 clear feedback; review PASS; deploy GIS-only PASS; **QA PASS** «**QA ROUTING-POINT-COORD-EDIT-A PASS operatore**»; display **`B6.3RPC-A-FIX1 · build 95`**; superseded live da TRACK-POINT-CENTER.
  - **MAP-CENTER-VIEWPORT-AWARE-A (+ FIX1–FIX3) build 93 — CLOSED / PASS end-to-end:** tip storico **`d0688ea`**; blob **`55d414bc…`**; byte LF **3149321**; SHA-256 LF **`0c23594c…`**; centra usable + costi normalizzati; review FIX3 PASS; deploy GIS-only PASS; **QA PASS** «**QA MAP-CENTER-VIEWPORT-AWARE-A-FIX3 PASS operatore**»; display **`B6.2MCV-A-FIX3 · build 93`**; superseded live da tip `0482ef8`.
  - **TRACK-PROFILE-POINTS-DISPLAY-A build 89 — CLOSED / PASS end-to-end:** tip storico **`3838e9e`**; blob **`48abde62…`**; byte LF **3144095**; SHA-256 LF **`464eed94…`**; overlay punti Profilo max 120 distance-decimated; review downstream PASS; deploy GIS-only PASS; **QA PASS** «**QA TRACK-PROFILE-POINTS-DISPLAY-A PASS operatore**»; display **`B6.2TPD-A · build 89`**; superseded live da MAP-CENTER.
  - **APP-BUILD-LABEL-UX-A (+ FIX1) build 88 — CLOSED / PASS end-to-end:** catena **`6de0e98`** (87) → tip storico **`da3397b`** (88 FIX1); blob **`f028f390…`**; byte LF **3139603**; SHA-256 LF **`49d4db86…`**; HUD testuale rimossa + footer GIS stabile; QA FAIL A → FIX1; deploy GIS-only PASS; **QA PASS** «**QA APP-BUILD-LABEL-UX-A-FIX1 PASS operatore**»; display **`B6.2BL-A-FIX1 · build 88`**; superseded live da TPD-A.
  - **OUTDOOR-ROUTING-POINT-UNDO-A / OUTDOOR-ROUTING-UNITS-A — CLOSED / ASSORBITI** in ROUTING-UX-POLISH-BUNDLE-A (+ FIX1).
  - **ROUTING-UX-POLISH-BUNDLE-A (+ FIX1) build 86 — CLOSED / PASS end-to-end:** catena **`7653ee7`** (85) → tip storico **`173b6cb`** (86 FIX1); blob **`9686245e…`**; byte LF **3150227**; SHA-256 LF **`4c197243…`**; undo storico + unità session + feedback + badge + focus; deploy GIS-only PASS; **QA PASS** «**QA ROUTING-UX-POLISH-BUNDLE-A-FIX1 PASS operatore**»; display **`B6.2UX-A-FIX1 · build 86`**; superseded live da APP-BUILD-LABEL → TPD-A.
  - **TRACK-SAVE-AS-NAME-A build 83 — CLOSED / PASS end-to-end:** tip storico **`8a641bc`**; display **`B6.1TSN-A · build 83`**; superseded live da RSD-A.
  - **OUTDOOR-ROUTING-F-AVOID-AREAS-A (+ FIX1) build 219 — CLOSED / PASS end-to-end:** catena **`12a7477`** (218) → tip **`5477a5e`** (219 FIX1); blob **`a823ae9b…`**; byte LF **10537443**; SHA-256 LF **`eb7a8aa0…`**; aree da evitare draw + `custom_model`; FIX1 preserve algorithm/alternative/round_trip; DELICATO; REVIEW GPT-SOSTITUTIVA PASS; deploy+ABQA PASS; **QA PASS** «**QA OUTDOOR-ROUTING-F-AVOID-AREAS-A-FIX1 PASS operatore**»; display **`OUTDOOR-ROUTING-F-AVOID-AREAS-A-FIX1 · build 219`**; Regola H; **superseded live** da **OUTDOOR-ROUTING-ORS-PROVIDER-A**.
  - **OUTDOOR-ROUTING-ORS-PROVIDER-A build 220 — CLOSED / PASS end-to-end:** catena HTML **`2687873`** → FIX1 infra tip **`cfee0e4`**; blob **`23fe93aa…`**; byte LF **10562488**; SHA-256 HTTP **`67d86081…`**; provider ORS opt-in (mai Auto); gateway HTTPS; secret server-side; DELICATO; REVIEW GPT-SOSTITUTIVA PASS; deploy+ABQA PASS; **QA PASS** «**QA OUTDOOR-ROUTING-ORS-PROVIDER-A PASS operatore**»; display **`OUTDOOR-ROUTING-ORS-PROVIDER-A · build 220`**; Regola H.
  - **Prossimo ordine operativo:** resto **Bundle F** / da scegliere. Runtime live monolite **OUTDOOR-ROUTING-ORS-PROVIDER-A** tip **`cfee0e4`** build **220**; GH VPS **V3**; helper **0.1.3**; **OUTDOOR-ROUTING-ORS-PROVIDER-A CLOSED**; **OUTDOOR-ROUTING-F-AVOID-AREAS-A (+ FIX1) CLOSED** (superseded live); **ROUTING-ANELLO-A (+ FIX1) CLOSED** (superseded live); **ROUTING-ACTION-ROW-UX-A CLOSED** (superseded live); **ROUTING-ANDATA-RITORNO-A CLOSED** (superseded live); **UI-MODAL-ERROR-FOCUS-A-FIX1 (+ FIX2) CLOSED** (superseded live); **ROUTING-MODAL-OPEN-EXPANDED-A (+ FIX1) CLOSED** (superseded live; geometria 680/0.98 preservata); **UX-SEARCH-ERROR-FOCUS-A CLOSED** (superseded live); **ROUTING-GEOCODE-SNAP-A CLOSED** (superseded live); **L10N-EN-FR-FREEZE-A CLOSED**; **ROUTING-ALTERNATIVE-ROUTES-A CLOSED** (superseded live); **QA-CHATGPT-3LINE-CURSOR-RULES-A CLOSED**; **QA-CHATGPT-3LINE-HANDOFF-PREF CLOSED**; **QA-OPERATOR-IT-ONLY-PREF CLOSED**; **Oggetti GIS FROZEN**; **INFRA-GH-1A/1B/1D CLOSED**; **Bundle E CLOSED**; **REVERSE-A CLOSED**; **TRACK-MODAL-DISPLAY-PREFS-A CLOSED**; **ELEVATION-STYLE-A CLOSED**; **TRACK-ELEVATION-PROFILE-A + FIX1–FIX3 CLOSED**; **TRACK-SAVE-AS-NAME-A CLOSED**; **ROUTING-SUMMARY-DEDUP-A CLOSED**; **ROUTING-UX-POLISH-BUNDLE-A (+ FIX1) CLOSED**; **APP-BUILD-LABEL-UX-A (+ FIX1) CLOSED**; **TRACK-PROFILE-POINTS-DISPLAY-A CLOSED**; **MAP-CENTER-VIEWPORT-AWARE-A (+ FIX1–FIX3) CLOSED**; **ROUTING-POINT-COORD-EDIT-A (+ FIX1) CLOSED**; **TRACK-POINT-CENTER-BUTTON-A CLOSED**; **MAJOR-3-b2 (+ FIX1) CLOSED**; **ROUTING-GEOCODING-MULTIROW-A (+ FIX1 + FIX2) CLOSED**.
  - **ROUTING-ANELLO-A (+ FIX1) build 115 — CLOSED / PASS end-to-end:** tip **`f718582`**; catena `4135737` (114) → FIX1; blob **`0ffb7b34…`**; byte LF **3347642**; SHA-256 LF **`0513e768…`**; `round_trip` multi-seed + re-gate/timeout/antimeridiano; DELICATO; review FIX1 PASS; deploy+QA PASS; Regola H; display **`ROUTING-ANELLO-A-FIX1 · build 115`**.
  - **ROUTING-ACTION-ROW-UX-A build 113 — CLOSED / PASS end-to-end:** tip storico **`dde5156`**; blob **`e999cafe…`**; byte LF **3309352**; SHA-256 LF **`53293444…`**; action strip unificata mode+actions; ROUTINE; harness 28/28; deploy+QA PASS; Regola H; display **`ROUTING-ACTION-ROW-UX-A · build 113`**; **superseded live** da **ROUTING-ANELLO-A-FIX1**.
  - **ROUTING-ANDATA-RITORNO-A build 112 — CLOSED / PASS end-to-end:** tip storico **`c1a6c89`**; blob **`0d8824e0…`**; byte LF **3308964**; SHA-256 LF **`71f7bb1b…`**; due POST `/route` sequenziali out-and-back; session-only `routeMode`; DELICATO; review PASS PRE-DEPLOY; deploy+QA PASS; Regola H; display **`ROUTING-ANDATA-RITORNO-A · build 112`**; **superseded live** da **ROUTING-ACTION-ROW-UX-A**.
  - **UI-MODAL-ERROR-FOCUS-A-FIX1 (+ FIX2) build 111 — CLOSED / PASS end-to-end:** tip storico **`5fc39e9`**; blob **`45b9132a…`**; byte LF **3293265**; SHA-256 LF **`da5e8f95…`**; catena `6d272d7` (FIX1 · 110) → FIX2; follow-up finding su **UI-MODAL-ERROR-FOCUS-A**; attenzione multi-riga layout-neutral; QA FAIL FIX1 → FIX2; Regola H; display **`UI-MODAL-ERROR-FOCUS-A-FIX2 · build 111`**; **superseded live** da **ROUTING-ANDATA-RITORNO-A**.
  - **ROUTING-MODAL-OPEN-EXPANDED-A (+ FIX1) build 109 — CLOSED / PASS end-to-end:** tip storico **`89a08fb`**; blob **`a1ad55e5…`**; byte LF **3287946**; SHA-256 LF **`235ea017…`**; catena `ae28eec` (A · 108) → FIX1; altezza max + width operativa 680; QA FAIL A → FIX1; review+deploy+QA PASS; display **`ROUTING-MODAL-OPEN-EXPANDED-A-FIX1 · build 109`**; **superseded live** da **UI-MODAL-ERROR-FOCUS-A-FIX2**.
  - **UX-SEARCH-ERROR-FOCUS-A build 107 — CLOSED / PASS end-to-end:** tip storico **`0b27e27`**; blob **`c56b4a35…`**; byte LF **3285428**; SHA-256 LF **`25988cb5…`**; bundle ROUTINE chiude **ROUTING-SEARCH-UX-A** + **UI-MODAL-ERROR-FOCUS-A**; harness 38/38; deploy+QA PASS; **superseded live**.
  - **ROUTING-GEOCODE-SNAP-A build 106 — CLOSED / PASS end-to-end:** tip storico **`d1e770e`**; blob **`204f901c…`**; byte LF **3266772**; SHA-256 LF **`98b1e507…`**; preflight `/nearest` + soglie 50/250 + conferma inline + marker snapped; review GPT-sostitutiva PASS; deploy GIS-only PASS; **QA PASS** «**QA ROUTING-GEOCODE-SNAP-A PASS operatore**»; display **`ROUTING-GEOCODE-SNAP-A · build 106`**; **superseded live**.
  - **L10N-EN-FR-FREEZE-A — CLOSED / PASS docs-only (governance trasversale, 2026-08-02):** IT attiva per nuove stringhe; EN/FR expansion frozen; i18n esistente preservato; unfreeze solo esplicito; rule `.cursor/rules/32-l10n-en-fr-freeze.mdc`; **non** feature runtime; **non** lavoro traduzione/migrazione; **non** apre Bundle F; all’epoca monolite tip **`0c078ae`** (ora superseded live da **`d1e770e`**).
  - **ROUTING-ALTERNATIVE-ROUTES-A (+ FIX1–FIX3) build 105 — CLOSED / PASS end-to-end:** catena **`0d14820`** (102) → **`2728ca2`** (103 FIX1) → **`ab432b7`** (104 FIX2) → tip **`0c078ae`** (105 FIX3); blob **`024986bc…`**; byte LF **3236322**; SHA-256 LF **`0770e72d…`**; alternative CH-first + UX + layout azioni; review+deploy+QA PASS «**QA ROUTING-ALTERNATIVE-ROUTES-A-FIX3 PASS operatore**»; display **`B6.6AR-A-FIX3 · build 105`**; **superseded live** da **ROUTING-GEOCODE-SNAP-A**.
...
**Serie 50 PolygonZ:** layer sorgente PolygonZ; pipeline ha scartato Z; CRS orizzontale = CRS84; significato/datum verticale Z = **NOT VERIFIED** (non dichiarare CRS 3D).

**Pipeline:** SHP→GeoJSON via Python esterno documentato (formato, DBF Latin-1→UTF-8, chiusura anelli, drop Z); nessun datum shift necessario (layer già WGS84 geographic); script non in repo → implementazione interna non completamente verificabile. **Non** scrivere accuratezza assoluta = 0: nessuno scarto da riproiezione datum risulta introdotto dalla pipeline documentata; accuratezza assoluta footprint / generalizzazione quadro **non quantificate**.

**Distinzione:** **(A)** geometria footprint = OGC:CRS84 lon/lat (ricerca/visualizzazione/Web Mercator); **(B)** carta/edizione fisica = datum/proiezione/reticolato/margini/tagli/scansione **NOT VERIFIED** per singola edizione. Una carta Roma40/ED50 può avere footprint catalografico moderno in WGS84.

**Controllo operatore foglio 232 S50 SESTRI LEVANTE:** NW embedded ≈ 9.332321 E / 44.399023 N; waypoint MGRS `32T NQ 26463 16249` ≈ 9.332304 E / 44.399044 N; scarto ≈ **2,7 m**. Nessun offset significativo Roma40/ED50 sul footprint; differenza vs foto/scansione può dipendere da prospettiva/deformazione/taglio/margini/georeferenziazione; verifica sul footprint, **non** prova datum edizione cartacea.

**Bordi densificati Serie 50:** molte geometrie hanno bordi densificati; causa esatta **non provata**; possibile rappresentazione geografica del bordo foglio; **non** attribuire coste/province senza ulteriori evidenze.
...
**Stato:** **CLOSED / PASS end-to-end** (2026-08-07). Tip storico **`c4d7db5`** / build **135** / `CARTO-ARCHIVE-MATCH-A-FIX2`. Catena `39ba407`→`84c9710`→`c4d7db5`. Store additivo `cartoArchiveRecords`; chiave `provider+series_id+chart_id`; CRUD + rollback; UI Indice IGM; FIX1 persistenza transazionale + ISO UTC; FIX2 chiusura editor post-Salva + notice flash 10s. Attestazione «**QA CARTO-ARCHIVE-MATCH-A-FIX2 PASS operatore**»; finito Regola H. Deploy GIS-only PASS (`?v=c4d7db5`). Limitazioni: matching foglio≠edizione CRS; no scansioni/file. **Superseded live** da **CARTO-IGM-AREA-ESC-RESTORE-A** tip **`764e661`**.

### CARTO-IGM-AREA-ESC-RESTORE-A — Esc ripristina Indice IGM in area-pick

**Stato:** **CLOSED / PASS end-to-end** (2026-08-07). Tip storico **`764e661`** / build **136** / `CARTO-IGM-AREA-ESC-RESTORE-A`. Flag session-only `_areaPickMinimizedByPicker`; restore solo se minimizzato dal picker; `stopImmediatePropagation` su entrambi i listener Esc area-pick; preserva risultati/selezione/overlay; rearm wheel/+/− invariato. Attestazione «**QA CARTO-IGM-AREA-ESC-RESTORE-A PASS operatore**»; finito Regola H. Deploy GIS-only PASS (`?v=764e661`). Bundle **DELICATO leggero**. **Superseded live** da **COORD-MODAL-FORMAT-COPY-A-FIX1** tip **`a0a6816`**.

### COORD-MODAL-FORMAT-COPY-A (+ FIX1) — Formato e copia coordinate nei modal
...
**Stato:** **CLOSED / PASS end-to-end** (2026-08-10). Tip **`a667f7455ca0cdf73e56ea5944832011639e32e4`** (`a667f74`) — `fix(map): use native Strava tile size for overzoom`; build **143** / `MAP-TRANSPARENT-OVERLAY-STACK-A-FIX4`. Catena `de8e053`(139)→`d42e3d2`(140 FIX1)→`5aaa54b`(141 FIX2)→`261fcdf`(142 FIX3)→`a667f74`(143 FIX4). Blob **`db1b6f24…`**; byte LF **4809183**; SHA-256 LF **`50e9bedd…`**. FIX4: crop Strava dimension-aware (source 512×512 → crop corretto a z12+). Attestazione «**QA MAP-TRANSPARENT-OVERLAY-STACK-A-FIX4 PASS operatore**»; finito Regola H. Deploy GIS-only presupposto da QA (Cursor non ha ripetuto smoke in chiusura docs).
...
**Product scope v1 LOCKED — IN:** OSM Standard sharding; Google Satellite via `gsat`; WayMarked Hiking; Strava Run maxZoom 11; Hillshade maxZoom 12. **DEFERRED:** Slope; labels-only. **DROPPED:** OpenMapSurfer Terrain; Bing Hybrid as labels overlay.
...
**Stato:** **CLOSED / PASS docs-only** (2026-08-07). Riconciliazione diagnostic + scope + proxy; promozione OPEN / READY FOR IMPLEMENTATION; monolite invariato.

### DOCS-BACKLOG-WAYPOINT-CENTER-MAP-OVERLAYS-A — Registrazione backlog center + overlay

**Stato:** **CLOSED / PASS docs-only** (2026-08-07). Solo registrazione storica **WAYPOINT-EDITOR-CENTER-A** + overlay (all’epoca NOT OPENED); stato vivo overlay superseduto da **DOCS-MAP-TRANSPARENT-OVERLAY-STACK-A-OPEN**.

### CARTO-INDEX-FEDERATED-A — Indice cartografico federato e catalogo archivio personale

**Stato:** **OPEN / SEARCH-ENGINE CLOSED / UI-RESULTS CLOSED / ARCHIVE CLOSED / ESC CLOSED / COORD CLOSED / SERIES CLOSED / IIM SNAPSHOT CLOSED — NEXT PROVIDER** (2026-08-19). Work Unit: [`WU-0012-carto-index-federated.md`](WU-0012-carto-index-federated.md). Discovery-1 + ACQUIRE-A + licenza IGM (**Prot. IGM-2024-7891**) + SEARCH/UI/ARCHIVE/ESC/COORD CLOSED + **`CARTO-IGM-SERIES-EXPAND-A` (+ UX1–UX3-FIX3) CLOSED** + **`CARTO-IIM-PROVIDER-A-FIX1` CLOSED / PASS** LIVE `f90c503` / build **231**. Macro-feature **non** CLOSED: restano CIGA / UKHO DISCOVERY BLOCKED / online update. Sequenza interna WU **riprendibile** (altri provider). **Nessun** auto-start.

**Ordine candidato WU-0012 (riprendibile):** (1) CIGA · UKHO **NOT OPENED / DISCOVERY BLOCKED** · **CARTO-ONLINE-UPDATE-A** **NOT OPENED**; (2) **MODAL-OPEN-TOP-ALIGN-A** (backlog UX); (3) **CARTO-SEARCH-FILTER-LABEL-UX-A** (backlog UX — **NOT OPENED**, casa [`WU-0012` §15j](WU-0012-carto-index-federated.md)). Finding QA 230 filtro IIM: [`WU-0012` §15i](WU-0012-carto-index-federated.md) — **risolto** da FIX1 **CLOSED** §15k. **MAP-ZOOM-FOCUS-ANCHOR-A (+ FIX1) CLOSED**. **WAYPOINT-EDITOR-CENTER-A (+ FIX1–FIX3 + FIX3-FIX1) CLOSED**. **CARTO-IGM-SERIES-EXPAND-A (+ UX1–UX3-FIX3) CLOSED**. **COORD-MODAL-FORMAT-COPY-A (+ FIX1) CLOSED**. **CARTO-IGM-AREA-ESC-RESTORE-A CLOSED**. **CARTO-ARCHIVE-MATCH-A (+ FIX1–FIX2) CLOSED**. **CARTO-IGM-CRS-AUDIT-A** = DIAGNOSTIC COMPLETE / CRS AUDIT PARTIAL.
...
**Backlog QA build 183 (2026-08-14):** registrato sotto. Candidato **A** CLOSED in [`WU-0015`](WU-0015-dflight-hit-test.md). Candidato **B** **CLOSED / PASS** → [`WU-0016`](WU-0016-dflight-ux-coherence.md). Candidato **C** **CLOSED / PASS** → [`WU-0017`](WU-0017-dflight-atm09-visual-parity.md). Candidato **D** **CLOSED / PASS** → [`WU-0018`](WU-0018-dflight-atm09-legend-ux.md). E–H restano **NOT OPENED**.

### D-Flight — backlog emerso QA build 183 — 2026-08-14

**Stato:** candidato **C** **CLOSED / PASS** (WU-0017). Candidato **D** **CLOSED / PASS** → [`WU-0018`](WU-0018-dflight-atm09-legend-ux.md) (RULE-META-PROBE-A **BLOCKED**). E–H **NOT OPENED**. Registrazione prodotto/UX emersa in QA umana sul runtime live **build 183** / `D-FLIGHT-TEMPORAL-FILTER-UI-A-FIX3` / task `20b1b49`.
...
**Stato:** **CLOSED / PASS** in [`WU-0019-dflight-panel-side-by-side.md`](WU-0019-dflight-panel-side-by-side.md) — blocco `D-FLIGHT-PANEL-SIDEBYSIDE-IMPL-A-FIX4` · LIVE `9820c8a` / build **205** · QA operatore PASS · finito Regola H (2026-08-16).
...
**Stato:** **BACKLOG / NOT OPENED**. Registrazione docs-only `BACKLOG-MAP-UX-DFLIGHT-CLEANUP-A`. **Non** aperti. **Non** bloccanti. **Non** toccano G-D, F, WU-0012, WU-0013 (chiusa), Oggetti GIS (**FROZEN A TEMPO INDETERMINATO**).
...
| **GIS-POLYGON-METRICS-COMPACT-FORMAT-A** | Area/perimetro/lati con **1** decimale (solo presentation) | **CONSUMED / CLOSED** con bundle `GIS-WAYPOINT-POLYGON-UI-MAINTENANCE-A` LIVE **248** · QA PASS · Regola H |
...
| **GIS-WAYPOINT-TEXT-EXPORT-CLIPBOARD-A** | Export/clipboard testo semplice waypoint (Nome + coord formato modal) | **FIX1 PENDING QA** (2026-08-24) — parent **251** FAIL caso 2 (copia multi); candidate **252** `4ad3b52` FIX1; Oggetti GIS FROZEN altrove |
...
**Stato:** **CLOSED / PASS** build **245** `MAP-CENTER-VIEWPORT-AWARE-A-FIX1` · tip `03a222e429905477d4a288c4ba7cc5b986f08bff` · blob `b9258d757fd8bba291e4506680ba579a480f5c56`. Deploy + ABQA **PASS**. QA operatore **PASS** · finito Regola H (2026-08-21).
...
**Delta 2026-08-21 implementati (ROUTINE):** altezza GIS utile + banda DOM (topbar/scala/footer); **FIX1** dock **sinistro** (QA FAIL su dock destro che copriva chrome); priorità ops/Coordinate vertici sopra lista salvati; durante draw/edit lista compatta; `Ctrl+Z` → `polygonRemoveLastDraftPoint` (no redo). Camera useful-rect / lifecycle DELICATO del piano originale: **non** in questo pass (resta backlog se riaperto).
...
#### D-FLIGHT-CLOSE-CLEANUP-A (+ FIX1) — **CLOSED / PASS** (build 235, 2026-08-20)

**Stato:** LIVE `4f004339c510c8848ffa0641908a487eeb3701c2` / **235** / `D-FLIGHT-CLOSE-CLEANUP-A-FIX1` · blob `d2b7e1cd…` · REVIEW GPT-SOSTITUTIVA **PASS** · deploy GIS + ABQA **PASS** · QA operatore **PASS** · auto-`finito` Regola H.

Catena: 234 (`ea83704`) QA operatore FAIL (residui ATM09 fino a pan) → FIX1 teardown sincrono preferred OFF + `InvalidateVisual` prima di `SetOverlayVisible`/`renderTileMap`.
...
**Stato:** **CLOSED / PASS** end-to-end (2026-08-21). LIVE tip `ccb41668576686bd96ada4900e11e21f0e07ad3d` · build **243** · `GIS-POLYGON-VERTEX-COORD-UX-A-FIX4` · blob `04cfdfcc1eed8979e60b9ff176f93ceee79ccfcb` · reviewed immutable `5857cbb2c3fc73e688ae26c1e2a359bb76199416`. Review GPT-sostitutiva PASS · deploy GIS PASS · ABQA 17/17 PASS · **QA operatore PASS** · finito Regola H. Evidence: [`../orchestrator/inbox/2026-08-21_1945_GIS-POLYGON-VERTEX-COORD-UX-A-FIX4_deploy-abqa.md`](../orchestrator/inbox/2026-08-21_1945_GIS-POLYGON-VERTEX-COORD-UX-A-FIX4_deploy-abqa.md) · finito [`../orchestrator/inbox/2026-08-21_2035_riepilogo_finito-GIS-POLYGON-VERTEX-COORD-UX-A-FIX4.md`](../orchestrator/inbox/2026-08-21_2035_riepilogo_finito-GIS-POLYGON-VERTEX-COORD-UX-A-FIX4.md). FIX3 human FAIL (draft verts non trascinabili) resolved by FIX4.

#### GIS-POLYGON-METRICS-COMPACT-FORMAT-A

**Stato:** **CONSUMED / CLOSED** con bundle `GIS-WAYPOINT-POLYGON-UI-MAINTENANCE-A` (2026-08-22). LIVE tip `aa6e8f5…` · build **248** · blob `dadbf8af…`. QA operatore PASS · finito Regola H. Storico backlog + triage FAIL intermedio preservati.
...
**Stato:** **CONSUMED / CLOSED** con bundle `GIS-WAYPOINT-POLYGON-UI-MAINTENANCE-A` (2026-08-22). LIVE tip `aa6e8f5…` · build **248** · blob `dadbf8af…`. QA operatore PASS · finito Regola H. Storico backlog + triage FAIL intermedio preservati. Fix ROUTINE CSS/layout (no lifecycle modal).
...
**Stato:** **FIX1 / PENDING QA** (2026-08-24). Parent **251** QA FAIL caso 2 (Copia testo multi). Candidate **252** tip `4ad3b52` · blob `003b40b8…`. Deploy + ABQA PASS.  
...
Evidence backlog: [`../orchestrator/inbox/2026-08-21_1210_GIS-WAYPOINT-TEXT-EXPORT-CLIPBOARD-A-backlog.md`](../orchestrator/inbox/2026-08-21_1210_GIS-WAYPOINT-TEXT-EXPORT-CLIPBOARD-A-backlog.md). Parent: [`../orchestrator/inbox/2026-08-24_1042_GIS-WAYPOINT-TEXT-EXPORT-CLIPBOARD-A_deploy-abqa.md`](../orchestrator/inbox/2026-08-24_1042_GIS-WAYPOINT-TEXT-EXPORT-CLIPBOARD-A_deploy-abqa.md). FIX1: [`../orchestrator/inbox/2026-08-24_1500_GIS-WAYPOINT-TEXT-EXPORT-CLIPBOARD-A-FIX1_deploy-abqa.md`](../orchestrator/inbox/2026-08-24_1500_GIS-WAYPOINT-TEXT-EXPORT-CLIPBOARD-A-FIX1_deploy-abqa.md).
...
HTTP 200
byte repo/servito = 2606270 / 2606270
SHA-256 file = 849bf44f9b3eac9de547ebfeb040fb2d812968df43785eb722a757293b504b14 (CMP_PASS)
goi-gis-app.service = active/enabled (bind 100.114.7.53:8000)
restart = non necessario (http.server serve WorkingDirectory)
```
...
> **Snapshot storico post-IMPORT-DROP-A (superato — non coda attuale):** all’epoca il backlog correlato elencava IMPORT-DROP-B (KMZ), TRACK-MODAL-UX-A, TRACK-STYLE-A, TRACK-BRUSH-A (+ finding F1–F7 note-only). **Oggi:** IMPORT-DROP-B + TRACK-MODAL-UX-A **CLOSED** (`1d28163`); TRACK-STYLE-A **CLOSED** (`40c97b6`); TRACK-BRUSH-A **CLOSED** (`d4f877a`); TRACK-BRUSH-ANTIMERIDIAN **CLOSED / PASS** (`9cc7937`). Finding F1–F7 restano note-only. **Coda viva:** prossimo blocco **da scegliere**; backlog non aperto: **OFFLINE-DOWNLOAD-CONTROLS**, estensioni MAJOR-3/4 import (OM §7).

#### MAJOR-4-a — Mission Package export (build 33)

**Stato:** **CLOSED / PASS end-to-end** (2026-07-01).

**Runtime:** `73269a064365e4a93ad4f68d1f8b1fb52daea7e0` — blob git `0da1faa5a95410947340fadc9def320acb199566`.

**Scopo:** export Mission Package JSON nel Workbench — snapshot read-only `GOI_GIS_MISSION_PACKAGE` v1 con GeoJSON embedded; riuso selezione categorie MAJOR-3-a; helper `gisMissionPackageBuild`, `gisMissionPackageExportRun`; download `mission-package-*.json`.

**Gate:** bundle **ROUTINE** (export read-only; nessun nuovo campo persistito).

**Esclusi:** import/restore mission package, sanitizer/schema nuovi.

**Review:** **GPT/GLM sostitutiva PASS** pre-deploy (Claude offline; **non** review byte Claude).
...
HTTP 200
byte repo/servito = 2650883 / 2650883
SHA-256 file = 5bc551d952f03aa02565e4cefe844624fc63f52c64283afcbbe308a3cf35ee76d (CMP_PASS)
goi-gis-app.service = active/enabled (bind 100.114.7.53:8000)
```

**QA operatore:** PASS — «**QA MAJOR-4-a PASS operatore**».

**Runtime autorevole live VPS:** `73269a0`.

**Prossimi candidati:** **OFFLINE-DOWNLOAD-CONTROLS** backlog; estensioni **MAJOR-3** (import) backlog basso; **MAJOR-4** import/restore backlog basso.
...
**Esclusi:** import unificato, sanitizer/schema nuovi.
...
**Runtime autorevole live VPS:** `c0fb4da` (superseded da `73269a0`).

**Prossimi candidati:** estensioni **MAJOR-3** (import) backlog basso; **MAJOR-4** import/restore backlog basso.
