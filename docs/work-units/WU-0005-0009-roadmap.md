...
| A2 pannello scompare al tap mappa | **parziale** — **A2-B1** CLOSED/PASS (`db2f6ea`); **A2-B2** storico PARTIAL FAIL superseded (`cb9f92f`); **A2-B2-FIX** CLOSED/PASS (`70ed7b3`); **A2-B3** runtime `87cbe64`, deploy/QA pending |

**A1:** rimossa `renderAllMaps()` da `polygonRefreshEditUi`; `polygonScheduleEditOverlayRefresh` (RAF+token+guardie) → `renderTileMap` diretto (deviazione ratificata vs `refreshTileMapForTrackUi` — precedenza `viewCenter`→`lastPoint`); review Claude PASS; deploy VPS PASS; **QA operatore PASS** («QA POLY-UX-STABILITY-A1 PASS operatore»). **A2-B1:** CLOSED/PASS end-to-end (`db2f6ea`, deploy+QA PASS). **A2-B2:** rollback logico PASS; QA PARTIAL FAIL storico (overlay stale fino al pan); runtime `cb9f92f`; superseded da A2-B2-FIX. **A2-B2-FIX:** redraw sincrono post-close edit (`viewCenter`→`lastPoint`, `mapZoom||11`); runtime **`70ed7b3`**; deploy VPS PASS; **QA operatore PASS** («QA POLY-UX-STABILITY-A2-B2-FIX PASS operatore»). **A2-B3:** rimosso auto-arm in `openPolygonPanel()`; disegno esplicito via `#polygonPanelNewBtn`; runtime **`87cbe64`**; deploy/QA pending. Catena A2 **non** chiusa end-to-end finché A2-B3 deploy/QA non PASS.
