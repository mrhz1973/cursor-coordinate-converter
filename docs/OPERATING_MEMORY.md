...
- **Offline UX / cache per-layer / maxZoom (WU-0008 8d-B1):** **8d-B1-A PASS** (diagnosi) + **8d-B1-A-docs PASS** (roadmap). **8d-B1-B1 PASS** (`29ebf3a`) — badge «no offline», `OFFLINE_LAYER_IDS` catalogo-driven, pannello neutro, contatore cache nascosto su online-only, layer attivo verde. **8d-B1-B2 PASS** runtime (monolite, `finito` 2026-06-15) — stats IndexedDB reali tile/MB per-layer in `#pcLayerCacheStat` (`getLayerTileCacheStats` cursor prefisso `layerId:`); scan O(n) solo on-demand; `node --check` OK. **Prossimo candidato:** **8d-B** EOX oppure **8d-B1-B3** zoom-guard (fit-area z18). Prerequisito `OFFLINE_LAYER_IDS` soddisfatto.
