...
- **Offline UX / cache per-layer / maxZoom (WU-0008 8d-B1):** **8d-B1-A PASS** (diagnosi read-only) — IndexedDB `CoordConvMapTiles`/`tiles`, chiavi `layerId:z/x/y`, nessun indice; stats per-layer O(n) on-demand accettabile; badge «No offline» fattibile da `cacheable:false`; pannello neutro consigliato; maxZoom basemap già clampato, gap fit-area `Math.min(18,z)` e messaggio overlay. **8d-B1-A-docs PASS** — debiti registrati in roadmap: (1) `OFFLINE_LAYER_IDS` manuale vs `cacheable` — prerequisito EOX alta priorità; (2) fit-area hardcoded z18 — zoom-guard media priorità. **Prossimo candidato runtime:** **8d-B1-B1** badge + pannello neutro (+ fix `OFFLINE_LAYER_IDS` se possibile). EOX (**8d-B**) dopo prerequisiti.
