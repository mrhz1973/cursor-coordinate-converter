...
- **Measurement label collision avoidance — WU-0007 B4X1:** implementato runtime **`debd5b4`**; helper `mmEstimateLabelPillSize` / `mmMeasurePillDimsFromTextEl` / `mmLineLabelNormalOffset` in `renderMapMeasureOverlay` — offset normale dinamico su segmenti corti in pixel vs bbox pill reale (`getBBox` + `getComputedTextLength`); segmenti lunghi ≈ offset base 14px; **B4 righello PASS** (`54d8586`) invariato; **`APP_BUILD_ID` invariato (`B5.5Z`)** — QA via `?v=debd5b4`. **QA browser operatore: pending**. Dettaglio: roadmap §WU-0007 B4X1.
