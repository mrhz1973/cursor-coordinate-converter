...
- **Range Rings B6.5B-1 center handle visibility (2026-06-20):** PASS tecnico statico — runtime nel **commit di chiusura `finito`** di questa sessione (hash in orchestrator/inbox + `LAST_CURSOR_REPORT`); fix z-index confinato `.range-rings-overlay.rr-move-center-active{z-index:12}` (sopra pin 8/9 e wpt/trk 4/5, solo in move-center, linee `pointer-events:none`); handle ridisegnato **target/crosshair** (hit r=20, alone bianco r=11, anello scuro `#0f172a`, crosshair N/E/S/W, dot accento col set); classe modifier al wrap solo se handle reso (`rrHandleRendered`); controller drag invariato; build label **`B6.5B-1`**; **`node --check`** OK (2 blocchi). **Browser QA operatore:** pending post-deploy VPS `:8000`.
- **Prossimo candidato Range Rings:** deploy VPS B6.5B-1 + QA operatore (handle visibile/afferrabile); poi backlog post-B6.5.
