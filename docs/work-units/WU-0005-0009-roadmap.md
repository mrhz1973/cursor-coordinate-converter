...
**B5.5B export JPG avanzato — overlay mappa base (2026-06-20):** PASS tecnico statico — build **`B5.5B`**; `#jpgExportIncludeOverlays` default true; `exportMapAsJpg({ includeScale, includeOverlays })`; 4 overlay SVG WYSIWYG (`range-rings`/`saved-tracks`/`gis-polygons`/`waypoints`) via `rasterizeSvgOntoCanvas`; cursore escluso; scala B5.4d/B5.4eB invariata; no fetch; B5.5C–Z fuori scope. **`node --check`** OK. **QA operatore post-deploy: pending.**
