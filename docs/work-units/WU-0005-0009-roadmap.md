...
6. **WU-0010 — Outdoor Routing GraphHopper** (programma corrente, **OPEN / B1a+B1b CLOSED / B2 NEXT**) — vedi [`WU-0010-outdoor-routing-graphhopper.md`](WU-0010-outdoor-routing-graphhopper.md). Review upstream GLM **PASS CON CORREZIONI**; split **B1 + B2** ratificato; **B1a CLOSED** tip `d95f745` build 54; **B1b (+FIX1) CLOSED** tip `3a702e1` build 56; prossimo bundle autorizzabile **B2** (route GraphHopper; prompt Cursor + review downstream pre-deploy). Runtime live monolite **`ff43878`** build 59 (**TRACK-POINT-CAP-2000-FIX2**). **MAJOR-3-b2** resta parcheggiato.
...
- Blocchi futuri:
  ...
  - **OUTDOOR-ROUTING-GH-B1b (+ FIX1) build 56 — CLOSED / PASS end-to-end:** catena **`3fc67c7`** (55) → tip **`3a702e1`** (56 FIX1); blob **`15c57074…`**; byte LF **2868398**; SHA-256 LF **`789eaefd…`**; pick/marker/GPS + disarmo Routing↔BBOX; review PASS; deploy GIS-only Cursor SSH PASS tip **`3a702e1`**; **QA PASS** «**QA OUTDOOR-ROUTING-GH-B1b PASS operatore**»; **`APP_BUILD_NUM = 56`**. Superseded live da **TRACK-POINT-CAP-2000** `ff43878`. Prossimo routing **B2**.
  - **OUTDOOR-ROUTING-GH-B1a (+ FIX1 + FIX2) build 54 — CLOSED / PASS end-to-end:** catena **`53e25d6`** (52) → **`3760c77`** (53) → tip **`d95f745`** (54); blob **`06c83dff…`**; byte LF **2843944**; SHA-256 LF **`e7f985dc…`**; shell planner no-map; **QA PASS** «**QA OUTDOOR-ROUTING-GH-B1a PASS operatore**»; **`APP_BUILD_NUM = 54`**. Superseded live da **B1b** `3a702e1` (poi tip live **TRACK-POINT-CAP-2000**).
  - **TRACK-POINT-CAP-2000 (+ FIX1 + FIX2) build 59 — CLOSED / PASS end-to-end:** catena **`249df83`** (57) → **`c94297f`** (58 FIX1) → tip **`ff43878`** (59 FIX2); blob **`db0d669…`**; byte LF **2887395**; SHA-256 LF **`eb92eedc…`**; cap 2000 fail-closed; verify+rollback create; paginazione lista 50; notice GIS import cap; review FIX2 PASS CON OSSERVAZIONI; deploy GIS-only Cursor SSH PASS (HEAD doc `f584b5f`, monolite=`ff43878`); **QA PASS** «**QA TRACK-POINT-CAP-2000-FIX2 PASS operatore**»; **`APP_BUILD_NUM = 59`**. Runtime live VPS **`ff43878`**.
  - **Prossimo ordine operativo:** **OUTDOOR-ROUTING-GH-B2** (route GraphHopper; WU-0010 **OPEN / B1a+B1b CLOSED**). **MAJOR-3-b2** resta **parcheggiato**; **MAJOR-4** import/restore backlog basso; runtime live **TRACK-POINT-CAP-2000** tip **`ff43878`** build 59; **B1b CLOSED** `3a702e1` build 56; backlog UX track **TRACK-POINT-CENTER-BUTTON-A** (NON APERTO); micro-UX **non ora** (salvo bug reale).
