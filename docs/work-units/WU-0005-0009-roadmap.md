...
**B5.5Z-3 export JPG rapido pulsante superiore (2026-06-22):** PASS tecnico statico + review diff PASS — export geografico dal pulsante JPG; area automatica = viewport all'apertura; layer = `state.mapLayer`; zoom corrente→max esportabile singolo; overlay/scala/coordinate; Mappe Offline invariato; percorso entro-cap **invariato** post-DELTA-A1; **`node --check`** OK. **Superato per zoom oltre-cap da B5.5Z-DELTA-A1.**

**B5.5Z-DELTA-A1 export alto-zoom segmentato tile-only (2026-06-22):** PASS tecnico statico + review byte-level PASS + fix memoria deterministico PASS — tutti gli zoom interi fino a `layer.maxZoom`; entro-cap singolo JPG B5.5Z-3; oltre-cap segmenti tile-only sequenziali; stima preventiva + egress alta/hard-stop; overlay/scala/coordinate esclusi nei segmenti; cleanup canvas/blob deterministico; Mappe Offline/tile loader invariati; i18n IT/EN/FR; **`node --check`** OK; deploy/QA operatore VPS **pending**; **B5.5Z-DELTA-A1 non chiuso end-to-end**; **B5.5Z non chiuso end-to-end.** **Prossimo candidato:** deploy VPS + QA operatore quick export segmentato e regressione entro-cap/Mappe Offline.
