...
**B5.5Z-FIX0 offline-export JPG — `layer` undefined (2026-06-21):** PASS tecnico statico — bug preesistente: `exportOfflineAreaAsJpg()` leggeva `layer` senza dichiarazione in strict mode; fix: `const layer = getOfflineTileLayer(layerId)`; nessuna modifica cache/rete/OPSEC/progress/download; **`node --check`** OK; deploy/QA browser **non eseguiti**; prerequisito B5.5Z, **non** apertura overlay; **B5.5Z non completato**. **Prossimo candidato:** **B5.5Z-1** — modello geografico/proiezione offline-export, senza overlay.
