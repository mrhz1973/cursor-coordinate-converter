...
- **B5.5C export JPG — selezione granulare overlay + label waypoint (2026-06-21):** PASS tecnico remoto — runtime **`5a10a48`**; build **`B5.5C`**; dialog `#jpgExportDialog`: master overlay + opzioni subordinate transienti (tracce, waypoint, etichette waypoint, poligoni/aree, Range Rings) default tutte ON; master OFF esclude tutto e disabilita sub-opzioni conservando selezioni; waypoint OFF prevale su label; label OFF mantiene marker (`wpt-map-marker`); filtro su clone SVG export (`cloneMutator` in `rasterizeSvgOntoCanvas`); qualità fissa **3×** B5.5E-2 invariata; cap **8192** invariato; scala indipendente; nessuna persistenza; nessun fetch/tile/cache/proxy/OPSEC; **`node --check`** OK. **QA operatore post-deploy VPS: pending** (deploy blocco successivo).
