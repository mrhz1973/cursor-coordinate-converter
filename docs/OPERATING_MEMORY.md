...
- **Browse-cache guard `cacheable:false` (WU-0008 8d-B0):** PASS runtime (monolite) — `parseTileKeyLayerId` + guard fail-open in `cacheTileFromDisplay`: layer con `cacheable === false` (osmStandard, Esri 8c-B, futuri online-only) non persistono più in IndexedDB durante pan/zoom; layerId dalla `tkey` tile, non `state.mapLayer`; precache/export/precache guard invariati; `node --check` OK. Browser QA IndexedDB before/after: da confermare operatore. Prerequisito per 8d-B (EOX).
