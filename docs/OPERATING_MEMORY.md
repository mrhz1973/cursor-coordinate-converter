...
- **Range Rings B6.6B edit-mode center handle affordance (2026-06-20):** PASS tecnico statico — runtime **`97406ab`**; HEAD/deploy VPS **`63084dd`**; deploy GIS-only (Planet-Clone/proxy **non toccato**); smoke **`200`**, Content-Length **`2152189`**; build label **`B6.6B`** servita; QA tailnet `:8000/coordinate_converter%20Claude.html?v=97406ab`; handle `g.rr-center-handle` visibile per tutta Modifica (`_rrEditingSetId`); target/crosshair B6.5B-1 on-center + glifo muovi; drag-only; click-to-place gated su «Sposta centro sulla mappa»; **`node --check`** OK. **Browser QA operatore:** pending post-deploy (VPS aggiornato).
- **Prossimo candidato Range Rings:** QA operatore B6.6B; poi backlog post-B6.6B (es. restore pannello post-create se richiesto).
