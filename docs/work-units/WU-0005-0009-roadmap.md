...
- Blocchi futuri:
  ...
  - **P-POLYGON-LIST-UX-NEXT — backlog candidato (non WU aperta):** **(A)** rinomina inline sulla cella Nome (senza barra separata); **(B)** colonne tabella Poligoni ridimensionabili manualmente. Emerso da QA FIX2 — non blocca P-POLYGON-LIST-ENRICHMENT CLOSED.
  - **APP-BUILD-NUM-B1 — build number monotono runtime (metodo B) — CLOSED / PASS tecnico end-to-end:** runtime **`bd588a8`**, blob **`afddf87a…`**; `APP_BUILD_NUM = 1`; display **`B5.5Z · build 1`**; review byte Claude **PASS**; deploy GIS-only **PASS tecnico** (byte **2365479**, SHA **`23907b80…`**, CMP_PASS); verifica runtime minima display **PASS**; **nessuna QA operatore estesa**; **`APP_BUILD_ID` `B5.5Z` invariato**; runtime VPS live **`bd588a8`**. **Nota cleanup:** span statici footer/about → solo `B5.5Z` o vuoto al prossimo blocco runtime; `applyAppBuildLabel` unica fonte ` · build N` — fold con prossimo incremento `APP_BUILD_NUM`, non blocco separato obbligatorio.
  - **Prossimo ordine operativo:** (1) `docs/HANDOFF.md` method A; (2) UX-NEXT-A rinomina inline; (3) UX-NEXT-B colonne ridimensionabili.
...
**Nota cleanup (prossimo blocco runtime):** `#appBuildFooter` / `#appBuildAbout` hard-coded `B5.5Z · build 1` → riportare a `B5.5Z` o vuoto; `applyAppBuildLabel` unica fonte ` · build N`. Fold con prossimo incremento `APP_BUILD_NUM`.

**Prossimo ordine operativo candidato:** (1) `docs/HANDOFF.md` method A; (2) P-POLYGON-LIST-UX-NEXT-A rinomina inline; (3) P-POLYGON-LIST-UX-NEXT-B colonne ridimensionabili.
