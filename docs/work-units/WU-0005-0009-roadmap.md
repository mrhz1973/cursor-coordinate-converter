...
**Stato:** **BACKLOG / NOT OPENED** (docs-only 2026-08-07, `DOCS-BACKLOG-WAYPOINT-CENTER-MAP-OVERLAYS-A`). Intent: estendere Layers con overlay trasparenti opzionali **sopra** la basemap attiva (composito tipo SAS.Planet), senza sostituirla. Famiglie: **(A) LABELS-ONLY / ANNOTATION** (località, strade, POI, confini, etichette — non assumere endpoint Google/OSM labels-only utilizzabile; provider/endpoint/licenza/attribution/CORS/auth/ToS da diagnosi futura); **(B) THEMATIC TRANSPARENT** (es. Strava Heatmap — non labeling; stessa infrastruttura compositing; auth/ToS/fattibilità da verificare). Precedenti additivi: `SEAMARK_OVERLAY` / OpenSeaMap, `SONARCHART_OVERLAY` — valutare generalizzazione senza rompere layer esistenti; riferimento esterno Planet-Clone/SAS.Planet (non copiare architetture incompatibili). Contratto futuro minimo: basemap≠overlay; toggle indipendenti; multi-overlay se consentito; opacity per-layer da valutare; z-order deterministico; attribution; no cambio basemap su toggle; no fetch silenzioso avvio; rete solo se overlay attivo/consentito; OPSEC + `forceOffline` + `tileFetchAllowed`/gate; proxy/consenso se richiesto; cache offline separata per provider da progettare; metadata-only ≠ delete tile; offline solo per contratto futuro; no framework/npm. Categoria futura **DELICATO** — prima DIAGNOSTIC/PLAN read-only. **Fuori WU-0012** (non federated index). **Non** next; **non** runtime.

### DOCS-BACKLOG-WAYPOINT-CENTER-MAP-OVERLAYS-A — Registrazione backlog center + overlay

**Stato:** **CLOSED / PASS docs-only** (2026-08-07). Solo registrazione **WAYPOINT-EDITOR-CENTER-A** + **MAP-TRANSPARENT-OVERLAY-STACK-A**; monolite invariato; ordine next invariato.

### CARTO-INDEX-FEDERATED-A — Indice cartografico federato e catalogo archivio personale

**Stato:** **OPEN / SEARCH-ENGINE CLOSED / UI-RESULTS CLOSED / ARCHIVE CLOSED / ESC CLOSED / COORD CLOSED — NEXT SERIES|PROVIDER** (2026-08-07). Work Unit: [`WU-0012-carto-index-federated.md`](WU-0012-carto-index-federated.md). Discovery-1 + ACQUIRE-A + licenza IGM (**Prot. IGM-2024-7891**) + **`CARTO-SEARCH-ENGINE-A` CLOSED** + **`CARTO-UI-RESULTS-A` (+ FIX1–FIX3) CLOSED** + **`MAP-INTERACTION-CARTO-UX-BUNDLE-A` (+ FIX1–FIX5) CLOSED** + **`CARTO-IGM-RESULTS-UX-BUNDLE-B` (+ FIX1–FIX3) CLOSED** + **`CARTO-IGM-CRS-AUDIT-A` DIAGNOSTIC COMPLETE / CRS AUDIT PARTIAL** + **`CARTO-ARCHIVE-MATCH-A` (+ FIX1–FIX2) CLOSED** + **`CARTO-IGM-AREA-ESC-RESTORE-A` CLOSED** + **`COORD-MODAL-FORMAT-COPY-A` (+ FIX1) CLOSED / PASS end-to-end** (tip `a0a6816` / build 138). Macro-feature **non** CLOSED: restano espansione serie, provider successivi. Runtime live: `a0a6816` / `COORD-MODAL-FORMAT-COPY-A-FIX1 · build 138`. **MAP-TRANSPARENT-OVERLAY-STACK-A** è backlog Layers **fuori** questa WU.

**Ordine candidato post-COORD (nessun runtime auto-aperto):** (1) **CARTO-IGM-SERIES-EXPAND-A**; (2) provider successivi; (3) **MODAL-OPEN-TOP-ALIGN-A** (backlog UX). **Non** next (candidati futuri): **WAYPOINT-EDITOR-CENTER-A**; **MAP-TRANSPARENT-OVERLAY-STACK-A**. **COORD-MODAL-FORMAT-COPY-A (+ FIX1) CLOSED**. **CARTO-IGM-AREA-ESC-RESTORE-A CLOSED** (superseded live). **CARTO-ARCHIVE-MATCH-A (+ FIX1–FIX2) CLOSED**. **CARTO-IGM-CRS-AUDIT-A** = DIAGNOSTIC COMPLETE / CRS AUDIT PARTIAL.
