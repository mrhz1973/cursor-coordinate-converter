...
- **B5.5D-1 export JPG — riquadro coordinate punto/waypoint (2026-06-21):** PASS tecnico statico — build **`B5.5D`**; dialog `#jpgExportDialog`: sezione «Includi coordinate» (checkbox master **OFF** ad ogni apertura — OPSEC); sorgenti `state.viewCenter` / `state.lastPoint` / waypoint (`wp.id` canonico); formati primary (via `fmtPrimary`/`state.primary`), DD, DDM, DMS, UTM, MGRS (precisione **5**), DD+MGRS; snapshot immutabile pre-`await` in `exportMapAsJpg`; validazione fail-closed in dialog; `drawJpgExportCoords` top-left post-scala; indipendente da overlay granulare B5.5C e da scala; qualità fissa **3×** / cap **8192** invariati; nessuna persistenza coords; nessun GPS; **`node --check`** OK. **QA operatore post-deploy VPS: pending** → **B5.5D-2** deploy GIS-only + QA `?v=<hash runtime>`.
