...
6. **WU-0010 — Outdoor Routing GraphHopper** (programma corrente, **OPEN / B1a CLOSED / B1b NEXT**) — vedi [`WU-0010-outdoor-routing-graphhopper.md`](WU-0010-outdoor-routing-graphhopper.md). Review upstream GLM **PASS CON CORREZIONI**; split **B1 + B2** ratificato; **B1a (+FIX1+FIX2) CLOSED** tip `d95f745` build 54; prossimo bundle autorizzabile **B1b** (pick/marker/GPS; prompt Cursor + review downstream pre-deploy). Runtime live `d95f745` build 54. **MAJOR-3-b2** resta parcheggiato.
...
- Blocchi futuri:
  ...
  - **OUTDOOR-ROUTING-GH-B1a (+ FIX1 + FIX2) build 54 — CLOSED / PASS end-to-end:** catena **`53e25d6`** (52) → **`3760c77`** (53) → tip **`d95f745`** (54); blob **`06c83dff…`**; byte LF **2843944**; SHA-256 LF **`e7f985dc…`**; shell planner no-map; `state._routing` session-only; CTA/menu GraphHopper zero-rete; label editabili; minimize/resize GIS; review GPT PASS; deploy GIS-only Cursor SSH PASS tip **`d95f745`**; **QA PASS** «**QA OUTDOOR-ROUTING-GH-B1a PASS operatore**»; **`APP_BUILD_NUM = 54`**. Prossimo **B1b**.
  - **Prossimo ordine operativo:** **OUTDOOR-ROUTING-GH-B1b** (pick + marker + GPS; WU-0010 **OPEN / B1a CLOSED**). **MAJOR-3-b2** (apply additivo Import Hub) resta **parcheggiato** (non annullato); **MAJOR-4** import/restore backlog basso; **B1a CLOSED** `d95f745` build 54; **MAJOR-3-b1 CLOSED** `1812010` build 51; **OFFLINE-DOWNLOAD-CONTROLS-A CLOSED** `fb11986` build 50; **TRACK-CREATE-EDIT-UX-A CLOSED** `793f4cb` build 46; **TRACK-BRUSH-ANTIMERIDIAN CLOSED** `9cc7937` build 44; **TRACK-BRUSH-A CLOSED** `d4f877a` build 42; **TRACK-STYLE-A CLOSED** `40c97b6` build 38; **IMPORT-DROP-B + TRACK-MODAL-UX-A CLOSED** `1d28163` build 35; **IMPORT-DROP-A CLOSED** `5f57a75` build 34; **MAJOR-4-a CLOSED** `73269a0` build 33; **MAJOR-3-a CLOSED** `c0fb4da` build 32; programma pick Workbench **MAJOR-5A2 completo**; titolo statico `<title>` e micro-UX **non ora** (salvo bug reale).
