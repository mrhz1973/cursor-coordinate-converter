...
- Blocchi futuri:
  ...
  - **BATCH FEATURE POLIGONI (non chiuso):** (1) **P-UI-UNIFORM** **CLOSED / PASS end-to-end** — runtime **`e0e9578`**, blob **`edd4b973…`**, deploy+QA PASS («**QA P-UI-UNIFORM PASS operatore**»); Review Claude **NON RICHIESTA**; (2) **P-UNITS** (**`8c266ae`** + **`8be2845`** + **`11838a1`**) **CLOSED / PASS end-to-end** — blob **`10f5f1e…`**, deploy GIS-only **PASS tecnico** (HEAD VPS **`26f73f7`**, byte **2314390**, SHA-256 **`9b9aca0c…`**, cmp PASS); **QA operatore PASS** («**QA P-UNITS PASS operatore**»); lunghezza auto/m/km/NM/mi/ft; area auto/m²/ha/km²; session-only; isolamento Misura GIS/R&B; Review Claude **NON RICHIESTA**; (3) **P-VERTEX-MODAL** — runtime **`a4fa8e7`** + fix **`5f8f73d`** (lista Lati) + fix **`5449cb9`** (P-VERTEX-MODAL-FIX2 CSS `:not([open])` — dialog chiuso ma visibile per `display:flex`); deploy GIS-only **PASS tecnico**; **QA FAIL** su `a4fa8e7` (lista Lati) e su `5f8f73d` (header `×`/`−`); **QA re-test pending** su **`5449cb9`**; **non CLOSED end-to-end**; Review byte Claude `a4fa8e7` = **PASS** (retroattiva); FIX2 **RAMO A** (solo CSS); (4) **P-STYLE** — non iniziato, **review-gated**; batch separato **P5 / P5-B2-F / chiusura P5** invariato e **non chiuso**;
