...
- **Measurement label collision avoidance (backlog WU-0007 estensione futura, non implementato):** su segmenti corti in pixel, l’etichetta centrata/ruotata in `renderMapMeasureOverlay` (`.tile-measure-overlay`, `.mm-label`) copre linea/marker/freccia; comportamento desiderato: confronto lunghezza schermo vs bbox etichetta reale, offset lungo normale se insufficiente, ricalcolo su zoom/viewport/geometria, leader opzionale; **non** riaprire stato PASS WU-0007 B4. Dettaglio: roadmap §WU-0007 estensione backlog.
- **UX toolbar laterale:** WU-0007 PASS — B1/B2 pulsanti più piccoli + Layers allineato (`e4c2be3`), B3 GPS label + colore qualità PASS (`c051ee1`), B4 distanza → righello PASS (`54d8586`), B5 waypoint/posiziona/torna in gruppo espandibile PASS (`7a02a7e`), B6 poligoni dentro Tracce (`e8395e9`), B7 MGRS dentro Layers PASS (`74d3f32`), B8 Range & Bearing dentro Tracce (`fa12567`). Restano fuori da WU-0007 (backlog estensione futura, non WU aperta): collision avoidance etichetta misura su segmenti corti (vedi bullet dedicato sopra); eventuali standardizzazioni modal trasversali o refactor non ancora pianificati oltre la voce WU-0006.
