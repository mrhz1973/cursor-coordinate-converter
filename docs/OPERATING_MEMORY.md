...
- **B5.5E-2 export JPG — qualità fissa 3× senza selettore (2026-06-21):** PASS tecnico statico — build **`B5.5E-2`**; rimosso selettore UI 1×/2×/3× (HTML/CSS/i18n/helper); `JPG_EXPORT_REQUESTED_SCALE = 3`; confirm/fallback sempre `exportMapAsJpg({ scale: 3 })`; cap **8192** e downgrade fail-safe B5.5E invariati; overlay B5.5B-1 invariato; nessuno stato transiente scala. **`node --check`** OK. **QA operatore post-deploy VPS: pending.**
