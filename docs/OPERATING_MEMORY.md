...
- **B5.4eB scala in-app allineata a export JPG (2026-06-20):** PASS tecnico statico — build **`B5.4eB`**; `buildScaleBar` layout due colonne (ratio **`1:N`** sx centrata, tabella metrica/Nm dx); CSS `.tile-scale` box opaco coerente export; mid-label rimosse; degradazione Nm su narrow (CSS); **`drawJpgExportScale`/export JPG invariati**; **`node --check`** OK; **QA operatore pending** post-deploy VPS (parità in-app vs JPG + regressione export B5.4d + Range Rings B6.6B).
