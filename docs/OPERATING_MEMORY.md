...
- **Range Rings B6.4a-2 panel full-height (2026-06-20):** PASS tecnico + **PASS operatore post-deploy VPS** — runtime **`656dd13`**; HEAD/deploy **`7dd1a41`** (origin/main + VPS GIS clone); deploy **GIS-only** (`goi-gis-app` active; Planet-Clone/proxy **non toccato**); smoke **`200 text/html`**, Content-Length **`2142705`**; build label **`B6.4a-2`** servita (title/footer/About/`APP_BUILD_ID`); QA tailnet `:8000/coordinate_converter%20Claude.html?v=656dd13`; `_rangeRingsPanelLayoutOpts` (`defaultHeightFraction` **0.92**, `defaultHeightCap` **100000**, `topbarReserve` **104** → altezza da `fromReserve`; bottom-anchor posiziona top sotto topbar; **`node --check`** OK. **Attestazione operatore:** «tutto perfetto» (2026-06-20).
