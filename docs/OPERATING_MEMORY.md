...
- **B5.4e scala in-app vs export JPG (backlog export):** scala overlay **in-app** (`.tile-scale` / `buildScaleBar`) **non** impaginata/posizionata come la legenda nel JPG esportato (`drawJpgExportScale`); evidenza operatore post-QA B5.4d — export JPG OK, scala a schermo diversa; **non** è FAIL di B5.4d export; obiettivo futuro: allineare layout scala in-app al layout export corretto; runtime **non modificato** in questo blocco.
