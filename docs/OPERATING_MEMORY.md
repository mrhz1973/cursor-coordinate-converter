...
- **Range Rings B6.6B edit-mode center handle affordance (2026-06-20):** PASS tecnico statico — runtime nel **commit di chiusura `finito`** di questa sessione; handle `g.rr-center-handle` visibile per tutta la sessione Modifica (`state._rrEditingSetId`, non solo `_rrEditingMoveCenterMode`); target/crosshair B6.5B-1 on-center + glifo muovi cardinali; drag-only (centro reale invariato all'ingresso in Modifica); click-to-place resta gated su «Sposta centro sulla mappa»; build label **`B6.6B`**; **`node --check`** OK. **Browser QA operatore:** pending post-deploy VPS `:8000`.
