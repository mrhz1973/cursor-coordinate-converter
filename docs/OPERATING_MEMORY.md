...
| **BLOCCO ATTIVO** | D-FLIGHT-UX-COHERENCE-LEGEND-ATM09-UX-A |
...
| **NEXT** | QA umana ChatGPT → `QA D-FLIGHT-UX-COHERENCE-LEGEND-ATM09-UX-A PASS operatore` → auto-`finito`; poi **AGGIORNA-A** |
...
> B2 LEGEND-ATM09-UX-A: legenda ATM09 contestuale + pulse raster + scroll pannello. LIVE `0c0f97d` / **194**. Helper **0.1.3**. Preferred/fetch/gate invariati. QA umana PENDING.
> WU-0015 CLOSED / PASS. Candidati C–H restano NOT OPENED.

### 7.2 RECENT / POINTERS (rolling max ~5 — navigazione, non stato concorrente)

1. **D-FLIGHT-UX-COHERENCE-LEGEND-ATM09-UX-A** — deploy PASS · Automated Browser QA PASS · QA FINALE CHATGPT — PENDING — LIVE `0c0f97d` / **194**
...
5. **CONTEXT-BUDGET-GUARD** — docs-only PASS (AI-BOOT + OM §4)
